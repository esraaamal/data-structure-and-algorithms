...
You are making a grocery list for ingredients needed in the gruffalo crumble recipe, below. Rather than 
taking the entire recipe, you only want a list of the item names.

Write a function named listFoods that takes in the recipe and returns a new array of the food items without 
any amount or units. Just the name. For example, '1 cup flour' will return 'flour'.
...
const listFoods = (recipe) => {
    let result = [];
    // Solution code here...
    let leng = recipe.ingredients.length;
    for (let i = 0; i < leng; i++) {
        let ingredientsLength = recipe.ingredients[i].split(" ");
        let newIngredients = ingredientsLength.slice(2);
        let hatNew = newIngredients.join(" ");
        result.push(hatNew);
    }
    return result;
};
...
const stepActions = (recipe) => {
    let result = [];
    let leng = recipe.steps.length;
    for (let i = 0; i < leng; i++) {

        let step1 = recipe.steps[i].split(" ");
        let stepAction12 = step1.shift();
        result.push(stepAction12);
...
Write a function named removeLastCharacters that takes in a
 string and a number. The numberOfCharacters argument determines 
 how many characters will be removed from the end of the string.
  Return the resulting string.

If the numberOfCharacters argument is greater than the length of
 the input string, the function should return an empty string.
...
return str ;
    ...
    }else if(numberOfCharacters <=leng1){
return str.slice(0,leng1-numberOfCharacters);
    }
...
// let result;
//     for(let i=0 ;i<str.length ;i++){
//     let notVowels1=arr[i];
// if('aeiou'.includes(notVowels1)){
// result=arr.slice(i,i+1);
// }
//      }return result.join(""); // Solution code here...
let i=0;
let newString2="";
while(i<str.length){
    let newStr=str[i];
    if(!'aeiou'.includes(newStr)){
 newString2=newString2+newStr ;
    }
    i++;
